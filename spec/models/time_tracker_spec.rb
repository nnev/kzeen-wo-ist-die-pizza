...
describe TimeTracker, type: :model do
  def tt
  ...
  before do
    ...
    @basket.submitted_at = Time.now - 5.minutes
  end

  describe '#overdue?' do
    it 'is false, if there’s still time left' do
      @basket.submitted_at = Time.now - 1.minutes
